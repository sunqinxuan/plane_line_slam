...
namespace ulysses 
{
	SystemSLAMSim::SystemSLAMSim(const std::string &settingFile)
	{
		remove("SystemSLAMSim.txt");
...
//		global_map->setDebug(debug);
	...
	double SystemSLAMSim::track(const double &time_stamp)
	{
		fp.open("SystemSLAMSim.txt",std::ios::app);
		fp<<std::endl<<"****************************SystemSLAMSim::track()*******************************"<<std::endl;
...
//			scan->association()->printAssociatedFeatures(fp);
//			scan->saveAssociationsSim(folder);
//			std::cout<<"saved simulated association "<<std::endl;

		}

//		fp<<"simulated features"<<std::endl;
//		scan->printFeatures(fp);
//		scan->saveFeatures(folder);
...
//
		...
		{
			time_used=feature_matching->match(scan_cur);
			scan_cur->localize();
			...
			scan_cur->association()->evalulatePR(fa,precision,recall);
			fp<<"precision="<<precision<<std::endl;
			fp<<"recall="<<recall<<std::endl;
			if(!debug) scan_cur->savePRC(save_folder,fa);
			delete fa;
...
//		std::cout<<"scan_cur="<<scan_cur<<", "<<scan_cur->association_sim()<<std::endl;
	...
	/*
	bool SystemSLAMSim::trackCamera(const cv::Mat &rgb, const cv::Mat &depth, const double &timestamp,
...
//	void SystemSLAMSim::showScans(const cv::Mat &rgb, const cv::Mat &depth, const double &timestamp,
...
//	}
//
//	void SystemSLAMSim::saveTraj(const std::string &file)
//	{
...
//	}
	...
	/*
	void SystemSLAMSim::loadScan(Scan *scan, const cv::Mat &rgb, const cv::Mat &depth)
	{
