...
#
# Last modified:	2019-12-20 09:02
#
# Filename:		it_geometric_feature_matching.cpp
#
# Description: 
#
===============================================*/
...
namespace BaselessCliff
{
	...
	{
		consistent_model=new TransformModel;
		consistent_model->initializeModel(this);
	}

	void Node_InterpTree::deriveConsistentModel(TransformModel* model) 
	{
		consistent_model=new TransformModel;
		consistent_model->deriveFrom(model);
	}

	bool Node_InterpTree::updateConsistentModel(InterNodeConsistency *model) {return consistent_model->update(model);}
	double Node_InterpTree::error() { return consistent_model->transError()+consistent_model->rotError(); }

//	bool Node_InterpTree::updateConsistentModel(InterNodeConsistency *model) 
//	{ 
////		bool updated=false;
...
//	{}
	...
	bool ProblemRotation::solve()
	{
		...
		else if(delta_cur<0.02 && delta_ref<0.02)//(delta_cur<THRES_RAD && delta_ref<THRES_RAD)
		{
...
//			t=0.5*(t1+t2);
	...
	bool TransformModel::computeTransform(Node_InterpTree* node)
	{
		if(dbg) fp_IT<<std::endl<<"-----------------------TransformModel::computeTransform()-----------------------"<<std::endl;
...
//				std::cerr<<"aligning failure !"<<std::endl;
	...
	bool TransformModel::fuse(const SolutionSet *s)
	{
		if(dbg) fp_IT<<std::endl<<"-----------------------TransformModel::fuse()-----------------------"<<std::endl;
		if(s->empty) return false;
		if(solution_set->empty) {*solution_set=*s; return true;}
//		if(solution_set->rotDOF==1 && solution_set->transDOF==2)
//		else if(solution_set->rotDOF==1)
//		else if(solution_set->transDOF==1)
//		else if(solution_set->rotDOF==0 && solution_set->transDOF==0)
	}

	bool TransformModel::fuse(SolutionSet *s11, const SolutionSet *s, const SolutionSet *s1)
	{
		Eigen::Vector3d ec=s->rotation_dir_c; 
		Eigen::Vector3d er=s->rotation_dir_r;
		Eigen::Vector3d ec1=s1->rotation_dir_c; 
		Eigen::Vector3d er1=s1->rotation_dir_r;

		Eigen::Vector3d t0=s->translation; 
		Eigen::Vector3d t01=s1->translation;
		Eigen::Vector3d w=s->translation_direction; 
		Eigen::Vector3d w1=s1->translation_direction; 

		if(s->rotDOF==1 && s->transDOF==2)
		{
			if(s1->rotDOF==1 && s1->transDOF==2)
			{
				ProblemRotation problem(ec,er,ec1,er1);
				problem.solve(s11);
				if(dbg) {fp_IT<<"s'' rotation: "<<std::endl; s11->printRot(fp_IT);}
				if(dbg) fp_IT<<"w.cross(w')="<<w.cross(w1).norm()<<std::endl;
				if(w.cross(w1).norm()<THRES_RAD)
				{
					if(dbg) fp_IT<<"w.cross(w')=0"<<std::endl;
					Eigen::Vector3d delta=t0-t01;
					if(dbg) fp_IT<<"delta_t0.dot(w)="<<fabs(delta.dot(w))<<std::endl;
					if(dbg) fp_IT<<"delta_t0.dot(w')="<<fabs(delta.dot(w1))<<std::endl;
					if(fabs(delta.dot(w))<THRES_DIST && fabs(delta.dot(w1))<THRES_DIST)
					{
						if(dbg) fp_IT<<"delta_t0.dot(w)=0"<<fabs(delta.dot(w))<<std::endl;
						if(dbg) fp_IT<<"delta_t0.dot(w')=0"<<fabs(delta.dot(w1))<<std::endl;
						s11->empty=false;
						s11->transDOF=2;
						s11->translation=0.5*(t0+t01);
						s11->translation_direction=w+w1;s11->translation_direction.normalize();
						if(dbg) {fp_IT<<"s'' translation: "<<std::endl; s11->printTrans(fp_IT);}
					}
					else 
					{
						if(dbg) fp_IT<<"w.cross(w')!=0"<<std::endl;
				...
				else // if(w.cross(w1).norm()>THRES_RAD)
				{
					Eigen::Matrix<double,3,6> A;
					Eigen::Matrix<double,3,1> b;
					A.block<3,3>(0,0)=-Transform::skew_sym(w);
					A.block<3,3>(0,3)=Transform::skew_sym(w1);
					b=t0-t01;
					Eigen::Matrix<double,6,1> mu_mu1=A.fullPivLu().solve(b);
					if(dbg) fp_IT<<"[mu mu']="<<mu_mu1.transpose()<<std::endl;
					Eigen::Vector3d mu=mu_mu1.block<3,1>(0,0);
					Eigen::Vector3d mu1=mu_mu1.block<3,1>(3,0);
					Eigen::Vector3d t=t0+w.cross(mu);
					Eigen::Vector3d t1=t01+w1.cross(mu1);
					if(dbg) fp_IT<<"t="<<t.transpose()<<std::endl;
					if(dbg) fp_IT<<"t1="<<t1.transpose()<<std::endl;
					s11->empty=false;
					s11->transDOF=1;
					s11->translation=0.5*(t+t1);
					s11->translation_direction=w.cross(w1).normalized();
					if(dbg) {fp_IT<<"s'' translation: "<<std::endl; s11->printTrans(fp_IT);}
				}
			}
			else if(s1->rotDOF==1 && s1->transDOF==1)
			{
				ProblemRotation problem(ec,er,ec1,er1);
				problem.solve(s11);
				if(dbg) {fp_IT<<"s'' rotation: "<<std::endl; s11->printRot(fp_IT);}
				if(s11->rotDOF==0)
				{
					Eigen::Vector3d trans=t01+s11->rotation_matrix*w1; // t'
					if(dbg) fp_IT<<"s' constrained: "<<trans.transpose()<<std::endl;
					Eigen::Vector3d delta=t0-trans;
					if(dbg) fp_IT<<"delta_t="<<delta.transpose()<<std::endl;
					if(dbg) fp_IT<<"delta_t.dot(w)="<<fabs(delta.dot(w))<<std::endl;
					if(fabs(delta.dot(w))<THRES_DIST)
					{
						s11->empty=false;
						s11->transDOF=0;
						s11->translation=trans;
						if(dbg) {fp_IT<<"s'' translation: "<<std::endl; s11->printTrans(fp_IT);}
					}
				}
				else if(s11->rotDOF==1)
				{
					if(fabs(t0.dot(w)-t01.dot(w))<THRES_DIST)
					{
						s11->empty=false;
						s11->transDOF=1;
						s11->translation=s1->translation;
						s11->translation_direction=s1->translation_direction;
						if(dbg) {fp_IT<<"s'' translation: "<<std::endl; s11->printTrans(fp_IT);}
					}
				}
			}
			else if(s1->rotDOF==0 && s1->transDOF==1)
			{
				s11->rotDOF=0;
				s11->rotation_matrix=s1->rotation_matrix;
				s11->rotation_axis=s1->rotation_axis;
				s11->rotation_angle=s1->rotation_angle;
				if(dbg) {fp_IT<<"s'' rotation: "<<std::endl; s11->printRot(fp_IT);}
				if(dbg) fp_IT<<"w.dot(w1)="<<fabs(w.dot(w1))<<std::endl;
				if(fabs(w.dot(w1))<THRES_RAD)
				{
					if(dbg) fp_IT<<"w.dot(w1)=0"<<std::endl;
					Eigen::Vector3d delta=t0-t01;
					if(dbg) fp_IT<<"delta_t.dot(w)="<<fabs(delta.dot(w))<<std::endl;
					if(fabs(delta.dot(w))<THRES_DIST)
					{
						if(dbg) fp_IT<<"delta_t.dot(w)=0"<<fabs(delta.dot(w))<<std::endl;
						s11->empty=false;
						s11->transDOF=1;
						s11->translation=s1->translation;
						s11->translation_direction=s1->translation_direction;
				...
				else //if(fabs(w.dot(w1))>THRES_RAD)
				{
					Eigen::Matrix<double,3,4> A;
					Eigen::Matrix<double,3,1> b;
					A.block<3,3>(0,0)=-Transform::skew_sym(w);
					A.block<3,1>(0,3)=w1;
					b=t0-t01;
					Eigen::Matrix<double,4,1> mu_mu1=A.fullPivLu().solve(b);
					if(dbg) fp_IT<<"[mu mu']="<<mu_mu1.transpose()<<std::endl;
					Eigen::Vector3d mu=mu_mu1.block<3,1>(0,0);
					double mu1=mu_mu1(3);
					Eigen::Vector3d t=t0+w.cross(mu);
					Eigen::Vector3d t1=t01+mu1*w1;
					if(dbg) fp_IT<<"t="<<t.transpose()<<std::endl;
					if(dbg) fp_IT<<"t1="<<t1.transpose()<<std::endl;
					s11->empty=false;
					s11->transDOF=0;
			...
			else if(s1->rotDOF==0 && s1->transDOF==0)
			{
				s11->rotDOF=0;
				s11->rotation_matrix=s1->rotation_matrix;
				s11->rotation_axis=s1->rotation_axis;
		...
		else if(s->rotDOF==1 && s->transDOF==1)
		{
			if(s->rotDOF==1 && s->transDOF==1)
			{
				ProblemRotation problem(ec,er,ec1,er1);
				problem.solve(s11);
				if(dbg) {fp_IT<<"s'' rotation: "<<std::endl; s11->printRot(fp_IT);}
				if(s11->rotDOF==0)
				{
					Eigen::Matrix3d R11=s11->rotation_matrix;
					Eigen::Vector3d trans=t0+R11*w;
					Eigen::Vector3d trans1=t01+R11*w1;
					if(dbg) fp_IT<<"trans="<<trans.transpose()<<std::endl;
					if(dbg) fp_IT<<"trans'="<<trans1.transpose()<<std::endl;
					Eigen::Vector3d delta=trans-trans1;
					if(dbg) fp_IT<<"delta="<<delta.norm()<<std::endl;
					if(delta.norm()<THRES_DIST)
					{
						s11->empty=false;
						s11->transDOF=0;
						s11->translation=0.5*(trans+trans1);
						if(dbg) {fp_IT<<"s'' translation: "<<std::endl; s11->printTrans(fp_IT);}
					}
				}
				else if(s11->rotDOF==1)
				{
					Eigen::Vector3d nc=s11->rotation_dir_c;
					Eigen::Vector3d nr=s11->rotation_dir_r;
					Eigen::Vector3d delta_w=w-w1;
					Eigen::Vector3d delta_t=t01-t0;
					if(dbg) fp_IT<<"delta_w="<<delta_w.transpose()<<std::endl;
					if(dbg) fp_IT<<"delta_t="<<delta_t.transpose()<<std::endl;
					if(delta_w.norm()<THRES_DIST && delta_t.norm()<THRES_DIST)
					{
						s11->empty=false;
						s11->transDOF=1;
						s11->translation=0.5*(t0+t01);
						s11->translation_direction=0.5*(w+w1);
						if(dbg) {fp_IT<<"s'' translation: "<<std::endl; s11->printTrans(fp_IT);}
					}
					else if(fabs(delta_w.norm()-delta_t.norm())<THRES_DIST)
					{
						delta_w.normalize(); delta_t.normalize();
						ProblemRotation problem(nc,nr,delta_w,delta_t);
						problem.solve(s11);
						Eigen::Matrix3d R11=s11->rotation_matrix;
						Eigen::Vector3d trans=t0+R11*w;
						Eigen::Vector3d trans1=t01+R11*w1;
			...
			else if(s->rotDOF==0 && s->transDOF==1)
			{
				
			}
			else if(s->rotDOF==0 && s->transDOF==0)
			{}
		}
		else if(s->rotDOF==0 && s->transDOF==1)
		{
			if(s->rotDOF==0 && s->transDOF==1)
			{
		...
		{
			fp_IT<<std::endl;
			fp_IT<<"s:"<<std::endl; s->print(fp_IT);
			fp_IT<<"s':"<<std::endl; s1->print(fp_IT);
			fp_IT<<"s'':"<<std::endl; s11->print(fp_IT);
		}
	}

	using namespace std;
	bool InterpTree::construct(const std::vector<Feature*> &features_cur, const std::vector<Feature*> &features_ref,
								Scan* scan, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
...
//							attachNode=true; 
						...
						InterNodeConsistency *model=new InterNodeConsistency(node, node_interp);
						if(debug) fp_IT<<std::endl<<"--------------------------------------------------------"<<std::endl
									<<model<<std::endl;
//						if(debug) std::cout<<model<<std::endl;
						SolutionSet *solution_set=new SolutionSet;
						if(model->consistency(solution_set))
						{
							cout<<endl<<node<<", "<<node_interp<<endl;
							cout<<solution_set;
							vis->removeAllPointClouds();
							vis->removeAllCoordinateSystems();
							scan->vis(vis); scan->ref()->vis(vis);
							node->feature_cur->vis(vis,255,0,0,"n1_cur");
							node->feature_ref->vis(vis,0,0,255,"n1_ref");
							node_interp->feature_cur->vis(vis,255,0,255,"n2_cur");
							node_interp->feature_ref->vis(vis,0,255,255,"n2_ref");
							vis->spin();
...
//			}
	...
	double GeometricFeatureMatching::match(Scan *scan, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
		...
		double timeused;
		gettimeofday(&start,NULL);
		interp_tree->construct(features_cur,features_ref,scan,vis);
		gettimeofday(&end,NULL);
		timeused=(1000000*(end.tv_sec-start.tv_sec)+end.tv_usec-start.tv_usec)/1000;
