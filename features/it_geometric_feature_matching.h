...
#
# Filename:		it_geometric_feature_matching.h
#
# Description: 
#
===============================================*/
#ifndef _IT_GEO_FEATURE_MATCH_H_
#define _IT_GEO_FEATURE_MATCH_H_
...
//		}
		...
		TransformModel *consistent_model;
		void initConsistentModel();// {consistent_model->initializeModel(node);}
		void deriveConsistentModel(TransformModel* model);// {consistent_model->deriveFrom(model);}
		bool updateConsistentModel(InterNodeConsistency *model);// {consistent_model->update(model);}
...
//		bool spatialConsistent(Node_InterpTree* node);
	...
	struct SolutionSet
	{
		SolutionSet() : empty(true), rotDOF(0), transDOF(0) {}
		...
		// rotDOF
		Eigen::Vector3d rotation_rx, rotation_ry;
		Eigen::Vector3d rotation_dir_c, rotation_dir_r;
		...
		{
			os<<"empty="<<s->empty<<", rotDOF="<<s->rotDOF<<", transDOF="<<s->transDOF<<std::endl;
			return os;
		}

		void printRot(std::ostream &os) const 
		{
			if(empty) {os<<"empty set!"<<std::endl; return;}
			os<<"rotDOF="<<rotDOF<<std::endl;
			if(rotDOF==0)
			{
				os<<"rotation_axis="<<rotation_axis.transpose()<<std::endl;
				os<<"rotation_angle="<<rotation_angle<<std::endl;
			}
			else if(rotDOF==1)
			{
				os<<"rotation_dir_c="<<rotation_dir_c.transpose()<<std::endl;
				os<<"rotation_dir_r="<<rotation_dir_r.transpose()<<std::endl;
				os<<"rotation_rx="<<rotation_rx.transpose()<<std::endl;
				os<<"rotation_ry="<<rotation_ry.transpose()<<std::endl;
			}
		}
		
		void printTrans(std::ostream &os) const 
		{
			if(empty) {os<<"empty set!"<<std::endl; return;}
			os<<"transDOF="<<transDOF<<std::endl;
			if(transDOF==0)
			{
				os<<"translation="<<translation.transpose()<<std::endl;
			}
			else if(transDOF==1 || transDOF==2)
			{
				os<<"translation="<<translation.transpose()<<std::endl;
				os<<"direction="<<translation_direction.transpose()<<std::endl;
			}
		}
	...
	public:
		...
		bool solve();
		bool solve(SolutionSet* solution_set)
		{
			isConsistent=solve();
			if(isConsistent)
			{
				solution_set->empty=false;
				solution_set->rotDOF=(int)hasDOF;
				if(hasDOF)
				{
					solution_set->rotation_dir_c=nc;
					solution_set->rotation_dir_r=nr;
					solution_set->rotation_rx=nr.cross(nc).normalized();
					solution_set->rotation_ry=nr+nc; solution_set->rotation_ry.normalize();
				}
				else 
				{
					solution_set->rotation_axis=axis;
					solution_set->rotation_angle=angle;
					solution_set->rotation_matrix=getRotationMatrix();
				}
			}
			else solution_set->empty=true;
			return isConsistent;
		}
...
//			std::cout<<"-----getRotationMatrix-----"<<std::endl;
		...
		static double cos_theta(Eigen::Vector3d r, Eigen::Vector3d nc, Eigen::Vector3d nr)
		{
		...
		static double sin_theta(Eigen::Vector3d r, Eigen::Vector3d nc, Eigen::Vector3d nr)
		{
			double den=1.0-r.dot(nc)*r.dot(nr);
		...
		// rotation axis is r;
		static double rotation_angle(Eigen::Vector3d r, Eigen::Vector3d nc, Eigen::Vector3d nr) 
		{
			nc.normalize();
			nr.normalize();
...
//			std::cout<<"theta="<<theta<<std::endl;
	...
	public:
		...
		bool solve(SolutionSet* solution_set)
		{
			if(solution_set->empty) return false;
			isConsistent=solve();
			if(isConsistent)
			{
				solution_set->empty=false;
				if(solution_case==LIN1DOF) solution_set->transDOF=1;
				if(solution_case==LIN2DOF) solution_set->transDOF=2;
				if(solution_case==ROT1DOF) solution_set->transDOF=1;
				solution_set->translation=translation;
				solution_set->translation_direction=direction;
			}
			else solution_set->empty=true;
			return isConsistent;
		}
...
//		double THRES_RAD, THRES_DIST;
	...
	public:
		...
		InterNodeConsistency(Node_InterpTree* ni, Node_InterpTree *nj) : node_i(ni), node_j(nj) 
		{ 
			rotation_consistency=new ProblemRotation;
			translation_consistency=new ProblemTranslation; 
			isConsistent=false;
		...
		bool consistency(SolutionSet* solution_set)
		{
			empty=false;
			isConsistent=false;
			...
			isConsistent = rotation_consistency->solve(solution_set) && translation_consistency->solve(solution_set);
			ModelType();

			if(dbg) fp_IT<<std::endl<<"inter-node consistency => "<<isConsistent<<std::endl;
...
//		bool interConsistent(InterNodeConsistency *model) const 
//		{
...
//		}
		...
		bool update(const InterNodeConsistency* model)
		{
			empty=false;
		...
		friend std::ostream & operator << (std::ostream &os, const InterNodeConsistency *m)
		{
...
//		// update "this" with the input "model";
//		// return true if "this" is consistent with "model";
//		bool updateModel(const InterNodeConsistency* model);
	...
	public:
		...
		void pushModel(InterNodeConsistency* model) {models.push_back(model);}
		...
		{
			if(dbg) fp_IT<<std::endl<<"-----------------------TransformModel::initializeModel()-----------------------"<<std::endl;
			models.clear();
//			if(node->null)
//			{
//				rotation=new ProblemRotation;
...
//			else 
		...
		void deriveFrom(const TransformModel* model) 
		{
			models.clear();
...
//			models.push_back(model_binary);
	...
	private:
		...
		std::vector<InterNodeConsistency*> models;
...
//		double THRES_RAD, THRES_DIST;
		...
		bool fuse(const SolutionSet *s);
		// s''=fuse(s,s');
		bool fuse(SolutionSet *s11, const SolutionSet *s, const SolutionSet *s1);
